...
module.exports = function() {
  ...
  if(urls[process.platform]) {
    ...
    if(!fs.existsSync(fn)) {
      console.log(util.format('Downloading sauce-connect via url "%s"', url));
      request(url)
        .pipe(unzip.Extract({ path: dir }))
        .on('error', function(err) {
          var message = util.format('Error downloading "%s": %s', url, err);
        ...
        .on('close', function() {
          ...
          console.log('Installation of sauce-connect complete');
